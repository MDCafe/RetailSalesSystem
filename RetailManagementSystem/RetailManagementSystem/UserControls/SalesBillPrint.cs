                ...
                using (var headerFont = new Font("Maiandra GD", 8, FontStyle.Bold))
                {
                    using (var itemFont = new Font("Arial", 9, FontStyle.Regular))
                    {
                        ...
                        using (var solidBrush = new SolidBrush(Color.Black))
                        {
                            ...
                            {
                                using (var maiandraFont = new Font("Maiandra GD", 12, FontStyle.Bold))
                                {
                                    int headerStartX = startX + 130;
                                    //drawFormat.FormatFlags = StringFormatFlags.;

                                    e.Graphics.DrawString(_appDetail.Name, maiandraFont, solidBrush, headerStartX, startY + Offset, drawFormat);
                                    Offset += 16;
                                    ...
                                    using (var rightAlignformat = new StringFormat())
                                    {
                                        ...
                                        //Customer Name
                                        using (var fontArial = new Font("Arial", 11, FontStyle.Bold))
                                        {
                                            e.Graphics.DrawString(_customerName, fontArial, solidBrush, headerStartX, startY + Offset, drawFormat);
                                            Offset += 25;
